...
# Database is created automatically beside this app.py on first run.
# You do NOT need to open or manually create a school.db file.
...
# Admission enquiries always go to this address.
ADMISSION_RECEIVER = "ddmonishwaran@gmail.com"

# Gmail SMTP sender credentials are intentionally NOT hard-coded.
# Use a Gmail address + Gmail App Password as environment variables.
...
def submit_admission():
    ...
    applying_class = (data.get("applyingClass") or "").strip()
    guardian_name = (data.get("guardianName") or "").strip()
    contact_number = (data.get("contactNumber") or "").strip()
    message = (data.get("message") or "").strip()

    if not student_name or not applying_class or not guardian_name or not contact_number:
        return jsonify(error="Please fill all required admission fields."), 400

    # Store the enquiry on the server first.
    import datetime
    ...
    c.execute(
        """INSERT INTO admissions
           (student_name, applying_class, guardian_name, contact_number, message, created_at)
           VALUES (?,?,?,?,?,?)""",
        (student_name, applying_class, guardian_name, contact_number, message, created_at)
    )
    c.commit()
    ...
    # Send the details to the requested Gmail address.
    if not GMAIL_SENDER or not GMAIL_APP_PASSWORD:
        return jsonify(
            error="Admission was saved, but Gmail SMTP is not configured on the server."
        ), 503

    email = EmailMessage()
    email["Subject"] = f"New Admission Enquiry — {student_name} ({applying_class})"
    email["From"] = GMAIL_SENDER
    email["To"] = ADMISSION_RECEIVER
    email.set_content(
        f"""New Admission Enquiry

Student Name: {student_name}
Applying for Class: {applying_class}
...
"""
    ...
    try:
        ...
        app.logger.exception("Admission email failed: %s", exc)
        return jsonify(
            error="Admission was saved, but the email could not be sent."
        ), 502

    return jsonify(ok=True, sentTo=ADMISSION_RECEIVER)

@app.get("/health")
def health(): return jsonify(ok=True)

# Automatically creates school.db and all required tables/data.
# If school.db does not exist, SQLite creates it here.
init_db()
